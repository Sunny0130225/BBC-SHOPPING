<template>
  <div class="d-flex justify-content-center align-items-center vh-100 bg-light">
    <div class="card shadow p-4" style="width: 400px">
      <h3 class="text-center mb-4">BBC前臺登入</h3>
      <form @submit.prevent="handleLogin">
        ...
        <!-- Login Button -->
        <button type="submit" class="button w-100 mb-3">登入</button>
        <p v-if="error" class="text-danger text-center">{{ error }}</p>
      </form>
      <!-- Add Member Button -->
      <button @click="createMember" class="button w-100">註冊會員</button>
...
const handleLogin = async () => {
  ...
  error.value = "";
  // 登入邏輯
  const loginResp = await axios.post("/front/login", {
    email: email.value,
  ...
  if (loginResp.data.success) {
    ...
    if (detailsResp.data.verified == "false") {
      error.value = "帳號已被禁用，請洽管理員";
      console.log("登入失敗，帳號已被禁用，請洽客服信箱");
      return;
    }
    console.log("登入成功");
    ...
    router.push("/loginHome"); // 跳轉到登入後首頁
  } else {
    error.value = "帳號密碼錯誤";
  }
};
         ...
         color:white;
          ...
          <table>
              ...
              <td colspan="2" style="text-align: center;">
                <button id="generate-fake-data" type="button" class="button" style="margin: 20px 0px 0px 0px;">一鍵帶入</button>
              </td>
            </tr>
           </table>
    ...
    didOpen: () => {
      ...
      document
        ...
        .addEventListener("click", () => {
          document.getElementById("cuname").value = "小明";
          document.getElementById("cemail").value = "ming@gmail.com";
          formToJSON;
          document.getElementById("cpassword").value = "ming";
        });
    },
  }).then((result) => {
